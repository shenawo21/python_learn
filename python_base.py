...
#-- 内置str处理函数：
    str1 = "stringobject"
    str1.upper(); str1.lower(); str1.swapcase(); str1.capitalize(
    ...
    str1.isalnum(); str1.isalpha(); str1.isdigit(); str1.islower(
...
#-- 索引和分片：
    ...
    # 分片，第三个参数指定步长，如`S[1:10:2]`是从1位到10位没隔2位获取一个字符。
...
#-- 字符串转换工具：
    ...
    # 将整数转化为二进制/八进制/十六进制字符串，返回('0b1101', '015', '0xd')
...
#-- Python中的字符串格式化实现1--字符串格式化表达式
    ...
    x = 1.23456789
    # 对齐方式："1.234568e+00 | 1.234568 | 1.23457"
    "%e | %f | %g" % (x, x, x)
    # 对齐方式：'  1.23*1.23  *001.23* +1.23'
...
#-- Python中的字符串格式化实现2--字符串格式化调用方法
    ...
    "my {1[spam]} runs {0.platform}".format(
        sys, {'spam': 'laptop'})                 # 基于位置的键和属性
    "{config[spam]} {sys.platform}".format(
        sys=sys, config={'spam': 'laptop'})   # 基于Key的键和属性
    "first = {0[0]}, second = {0[1]}".format(
        ['A', 'B', 'C'])                       # 基于位置的偏移量
    # 具体格式化
...
#-- if/else三元表达符（if语句在行内）
    ...
    result = (a > 20 and "big than 20" or a >
              10 and "big than 10" or a > 5 and "big than 5")
...
"""函数例子----函数例子----函数例子----函数例子----函数例子----函数例子----函数例子----函数例子----函数例子----函数例子----函数例子----函数例子----函数例子"""
    ...
    execfile(filename)
    # 构造一个序列，等价于[item for item in iterable if
    # function(item)]，function返回值为True或False的函数
    filter(function, iterable)
        list(filter(bool, range(-3, 4)))  # 返回[-3, -2, -1, 1, 2, 3], 没有0
    hasattr(object, name)               # 判断对象object是否包含名为name的特性
    getattr(object, name[, defalut])   # 获取一个类的属性d
...
#-- 抽象超类的实现方法
    ...
    # (3)上述的两种方法还都可以定义实例对象 实际上可以利用@装饰器语法生成不能定义的抽象超类
        ...
        # 返回 TypeError: Can't instantiate abstract class Super with abstract
        # methods action
        x=Super()
